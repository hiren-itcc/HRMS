...
- [ ] **Step 1: Transcribe the record layout from the published specification**

**Do not write the field order from memory, and do not accept mine.** Open the
current Protean/NSDL *e-TDS/TCS File Format for Form 24Q (Regular)* matching the
FVU release you will validate against. Transcribe the four record types into the
constant below, replacing every `TODO_FIELD`, and set `FVU_SPEC_VERSION` to the
document's version string.

**This step is not complete while any `TODO_FIELD` remains.** If you cannot
obtain the specification, stop and report it — do not guess and do not go on to
Step 2. A plausible-looking wrong file is the precise failure ADR-001 exists to
avoid, because a wrong 24Q is *filed* and then needs a correction statement.

Start `apps/api/src/modules/payroll/tds-files.ts` with:

```ts
/** The File Format document this layout was transcribed from. */
export const FVU_SPEC_VERSION = 'TODO_FIELD';

/**
 * Field order per record type, transcribed from the specification above.
 *
 * Positional and delimiter-separated: the portal reads by index, so an
 * inserted field silently shifts every one after it. Keep these in the
 * document's order and never sort them.
 */
const RECORD_LAYOUT = {
  FH: ['TODO_FIELD'],
  BH: ['TODO_FIELD'],
...
import { readFileSync } from 'node:fs';
import { join } from 'node:path';
import { build24Q, type ChallanEntry, type Deductee, FVU_SPEC_VERSION } from './tds-files';
...
describe('build24Q', () => {
  it('records the spec version it was built against', () => {
    // If nobody transcribed a real layout this is still the placeholder, and
    // the output is not fit to file.
    expect(FVU_SPEC_VERSION).not.toMatch(/TODO_FIELD/);
  });
...
- [ ] **Step 5: Create the golden fixture, then read it**

```bash
cd apps/api && pnpm exec jest src/modules/payroll/tds-files.spec.ts -t golden
```

Expected: FAIL — fixture missing. Write it from the builder's own output:

```bash
mkdir -p apps/api/src/modules/payroll/__fixtures__
cd apps/api && pnpm exec jest src/modules/payroll/tds-files.spec.ts -t golden 2>&1 | head -40
```

Take the `Received` string from the failure, write it to
`apps/api/src/modules/payroll/__fixtures__/form24q-q2.txt`, then **read that
file and check it line by line against the specification before accepting it.**
A golden file nobody read is a snapshot of a bug — this repo has already shipped
tests that agreed with a defect (`employee-import.resolve.ts`).
...
pnpm exec biome check apps packages
git add apps/api/src/modules/payroll/tds-files.ts apps/api/src/modules/payroll/tds-files.spec.ts apps/api/src/modules/payroll/__fixtures__/
git commit -m "feat(payroll): build a Form 24Q file from challans and frozen payslips"
```
...
import { toMoney } from './payroll.mapper';
import { build24Q, type ChallanEntry, type Deductee } from './tds-files';
...
```ts
  describe('tds', () => {
    ...
    /*
     * On the rows, not the status code. `list` returns 200 for an empty
     * register as readily as for a full one, so a `where` that lost its
     * organizationId would sail through a status check and hand one tenant
     * another's deposits.
     */
    it('never returns another organization a challan', async () => {
      const res = await get('/payroll/challans', 'hr');
