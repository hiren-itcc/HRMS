...
**Never run `prisma migrate dev`, `migrate reset`, `db push` or any seed script
against the configured database.** It holds a real company's payroll. `db:migrate`
in `apps/api/package.json` is `prisma migrate dev` — do not use it.

No shadow database is configured in this repo (`SHADOW_DATABASE_URL` appears in
neither `.env.example` nor `schema.prisma`), so `--from-migrations` cannot work.
Diff the schema against its own previous git revision instead. **This touches no
database at all** — both inputs are files:

```bash
cd apps/api
git show HEAD:apps/api/prisma/schema.prisma > /tmp/prev.prisma
pnpm exec prisma migrate diff \
  --from-schema /tmp/prev.prisma \
...
export class TdsChallansService {
  ...
  private map(row: ChallanRow) {
    return {
      ...
      // `toDateKey`, not `.toISOString()`. Prisma hands this back as a Date
      // from a live query and as a string from a mocked one, and three sibling
      // payroll services already reach for this helper at exactly that seam.
...
describe('TdsReturnsService readiness', () => {
  it('refuses while the record layout is untranscribed', async () => {
    ...
    // Three published runs, so the substantive checks all pass and the only
    // thing left standing is the layout. Without these the month checks fire
    // first and this test would be asserting the wrong refusal.
...
export class TdsReturnsService {
  ...
  async readiness(
    ...
    if (missingRun.length || unpublished.length) {
      /*
       * Both refuse, reported apart because the remedy differs: one wants a
       * payroll run, the other wants the run published. `rowsFor` in
       * `statutory-filings.service.ts` draws the same line — NotFound for the
       * first, BadRequest for the second — and lets neither through. A month
       * with no run is not "nothing to report"; it is a month whose
       * deductions would be silently missing from a filed return.
       */
      const parts = [
        missingRun.length > 0 &&
...
- Produces:
  ...
  - `tdsKeys`, `tdsApi`, `TdsChallan`, `TdsPreview`, `TdsReturnRecord`, `QUARTERS`, `QUARTER_LABELS`, `formatTdsMoney`, `financialYearOf`
  - **No `readiness()` on `tdsApi`, deliberately.** The API's `preview` spreads its own readiness into the response, so `TdsPreview` already carries `blocked`, `layoutBlocked`, `warnings` and `months`. A second call would fetch the same answer twice and could disagree with itself.
  ...
  return (
    <div className="max-w-4xl space-y-5">
      ...
      */}
      ...
      {layoutBlocked && (
        <Alert variant="warning">
          {layoutBlocked} Everything below still reflects this quarter's real
          figures — if it reconciles now, it will still reconcile when the
          layout lands.
        </Alert>
      )}
      ...
      {preview.data && !blocked && (
        <section className="space-y-4 rounded-xl border p-4">
          ...
          {can('payroll.filing') && (
            <Button
              disabled={
                !!layoutBlocked || generate.isPending || preview.data.rowCount === 0
              }
...
**That automatic lift is the weak point, and it must be closed in the same
commit.** `LAYOUT_TRANSCRIBED` is a bare string comparison, so editing the
version alone flips the guard true whether or not anybody reordered a field —
"the version was bumped" is not evidence "the layout is right". Bind the flag
to something structural. Add to `tds-files.ts`:

```ts
/**
 * Record types whose field order came from the specification.
 *
 * `LAYOUT_TRANSCRIBED` used to be a string comparison alone, which meant
 * editing the version string unlocked statutory file generation without
 * anyone having touched a field order. Deriving it from the layout table
 * as well makes the claim structural: the guard cannot be lifted without
 * a layout actually being present.
 */
const RECORD_TYPES = ['FH', 'BH', 'CD', 'DD'] as const;

export const LAYOUT_TRANSCRIBED: boolean =
  FVU_SPEC_VERSION !== 'UNTRANSCRIBED' &&
  RECORD_TYPES.every((type) => (RECORD_LAYOUT[type]?.length ?? 0) > 0);
```

and a test asserting that every record `build24Q` emits carries exactly as many
fields as `RECORD_LAYOUT` declares for its type — which is what actually
catches a field added to `line([...])` without the table being updated:

```ts
  it('emits exactly the fields the layout declares, per record type', () => {
    const result = build24Q({
      ...base,
      challans: [challan('2026-07', 500)],
      deductees: { '2026-07': [person('EMP-1', 'ABCPD1234E', 500)] },
    });
    for (const record of result.content.trim().split('\n')) {
      const fields = record.split('^');
      const type = fields[0] as keyof typeof RECORD_LAYOUT;
      expect([type, fields.length]).toEqual([type, RECORD_LAYOUT[type].length]);
    }
  });
```

Export `RECORD_LAYOUT` so the test can read it.
