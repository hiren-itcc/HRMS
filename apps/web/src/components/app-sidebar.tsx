      ...
      {items.map((item) => {
        ...
        if (item.soon) {
          return (
            <span
              ...
              aria-disabled="true"
              className={cn(
                'flex cursor-not-allowed items-center gap-3 rounded-lg px-3 py-2 text-sidebar-foreground/40 text-sm',
        ...
        const link = (
          <Link
            ...
            className={cn(
              'flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-sm transition-colors duration-150',
              collapsed && 'justify-center px-2',
              active
...
export function AppSidebar() {
  ...
  return (
    <aside
      className={cn(
        'sticky top-0 hidden h-dvh shrink-0 flex-col bg-sidebar text-sidebar-foreground transition-[width] duration-200 ease-out lg:flex',
        collapsed ? 'w-16' : 'w-60',
      )}
    >
      <Link
      ...
      >
        ...
        {!collapsed && <span className="font-bold text-lg text-white tracking-tight">HRMS</span>}
      </Link>
      ...
      {user && !collapsed && (
        <div className="border-sidebar-border border-t px-5 py-4">
          <p className="truncate font-medium text-sm text-white">{user.email}</p>
